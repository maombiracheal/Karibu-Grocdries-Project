...
1. `cd KGL-Backend` from the workspace root (or simply operate from the root;
   the `package.json` is at the top level).
...
4. Seed the user database (creates director/manager/agent accounts):
   ```sh
   node KGL-Backend/scripts/seed.js
   # or npm run seed:users (if you add/adjust the package script)
   ```
5. Start the server:
   ```sh
...
- The `debugRoutes.js` file (temporary) can be used to inspect registered
  Express routes.
- Mongoose models live in `KGL-Backend/models` and include validation logic.
